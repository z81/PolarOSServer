...
abstract class User implements ActiveRecordInterface 
{
    ...
    /**
     * The value for the icq field.
     * @var        string
     */
    protected $icq;

    /**
     * The value for the github field.
     * @var        string
     */
    protected $github;
    ...
    public function setBackground($v)
    {
        ...
        if ($this->background !== $v) {
            $this->background = $v;
            $this->modifiedColumns[UserTableMap::COL_BACKGROUND] = true;
        }
    ...
    public function setSkype($v)
    {
        ...
        if ($this->skype !== $v) {
            $this->skype = $v;
            $this->modifiedColumns[UserTableMap::COL_SKYPE] = true;
        }
    ...
    public function setIcq($v)
    {
        ...
        if ($this->icq !== $v) {
            $this->icq = $v;
            $this->modifiedColumns[UserTableMap::COL_ICQ] = true;
        }
    ...
    /**
     * Set the value of [github] column.
     * 
     * @param string $v new value
     * @return $this|\User The current object (for fluent API support)
     */
    public function setGithub($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }
        ...
        if ($this->github !== $v) {
            $this->github = $v;
            $this->modifiedColumns[UserTableMap::COL_GITHUB] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : UserTableMap::translateFieldName('Background', TableMap::TYPE_PHPNAME, $indexType)];
            $this->background = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 7 + $startcol : UserTableMap::translateFieldName('Skype', TableMap::TYPE_PHPNAME, $indexType)];
            $this->skype = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 8 + $startcol : UserTableMap::translateFieldName('Icq', TableMap::TYPE_PHPNAME, $indexType)];
            $this->icq = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 9 + $startcol : UserTableMap::translateFieldName('Github', TableMap::TYPE_PHPNAME, $indexType)];
            $this->github = (null !== $col) ? (string) $col : null;
            $this->resetModified();
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_BACKGROUND)) {
            $modifiedColumns[':p' . $index++]  = 'background';
        }
        if ($this->isColumnModified(UserTableMap::COL_SKYPE)) {
            $modifiedColumns[':p' . $index++]  = 'skype';
        }
        if ($this->isColumnModified(UserTableMap::COL_ICQ)) {
            $modifiedColumns[':p' . $index++]  = 'icq';
        }
        if ($this->isColumnModified(UserTableMap::COL_GITHUB)) {
            $modifiedColumns[':p' . $index++]  = 'github';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'skype':                        
                        ...
                        break;
                    case 'icq':                        
                        $stmt->bindValue($identifier, $this->icq, PDO::PARAM_STR);
                        break;
                    case 'github':                        
                        $stmt->bindValue($identifier, $this->github, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[7], $arr)) {
            $this->setSkype($arr[$keys[7]]);
        }
        if (array_key_exists($keys[8], $arr)) {
            $this->setIcq($arr[$keys[8]]);
        }
        if (array_key_exists($keys[9], $arr)) {
            $this->setGithub($arr[$keys[9]]);
        }
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_BACKGROUND)) {
            $criteria->add(UserTableMap::COL_BACKGROUND, $this->background);
        }
        if ($this->isColumnModified(UserTableMap::COL_SKYPE)) {
            $criteria->add(UserTableMap::COL_SKYPE, $this->skype);
        }
        if ($this->isColumnModified(UserTableMap::COL_ICQ)) {
            $criteria->add(UserTableMap::COL_ICQ, $this->icq);
        }
        if ($this->isColumnModified(UserTableMap::COL_GITHUB)) {
            $criteria->add(UserTableMap::COL_GITHUB, $this->github);
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setIcq($this->getIcq());
        $copyObj->setGithub($this->getGithub());
        if ($makeNew) {
            $copyObj->setNew(true);
    ...
    {
        $metadata->addPropertyConstraint('login', new Unique());
